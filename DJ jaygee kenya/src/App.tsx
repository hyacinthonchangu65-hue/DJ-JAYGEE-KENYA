...
const youtubeChannelEmbedUrl = "https://www.youtube.com/embed?listType=user_uploads&list=UCHl_tZljeMOXcqVzTvCYOxg"

function getYouTubeEmbedUrl(url: string) {
  try {
    const parsedUrl = new URL(url)
    if (parsedUrl.hostname === "youtu.be") return `https://www.youtube.com/embed/${parsedUrl.pathname.slice(1)}`
    if (parsedUrl.pathname.startsWith("/embed/")) return url
    if (parsedUrl.pathname.startsWith("/shorts/")) return `https://www.youtube.com/embed/${parsedUrl.pathname.split("/")[2]}`
...
function StandaloneApp({ onInstall, isInstalled }: { onInstall: () => void; isInstalled: boolean }) {
  ...
  const [watchingVideo, setWatchingVideo] = useState<{ title: string; url: string } | null>(null)
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] text-[#111111] pb-24">
      ...
      <main className="mx-auto max-w-xl px-4 py-5">
        ...
        {activeTab === "videos" && (
          <div className="space-y-5"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#A41E14]">Watch on YouTube</p><h1 className="mt-1 text-3xl font-bold">DJ JayGee TV</h1><p className="mt-2 text-sm text-gray-500">Watch performances, live streams, and YouTube Shorts inside the app.</p><a href="https://youtube.com/@djaygeekenya-c6q7i" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#A41E14] px-4 py-3 text-sm font-semibold text-white"><YouTubeIcon /> Open YouTube channel</a></div><div className="overflow-hidden rounded-2xl bg-[#111111] shadow-xl"><div className="aspect-video"><iframe src={youtubeChannelEmbedUrl} title="DJ Jaygee Kenya YouTube videos" className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div><p className="px-4 py-3 text-xs text-white/60">Latest channel videos play here inside the app.</p></div>{watchingVideo && getYouTubeEmbedUrl(watchingVideo.url) && <div className="overflow-hidden rounded-2xl bg-[#111111] shadow-xl"><div className={watchingVideo.url.includes("/shorts/") ? "mx-auto aspect-[9/16] max-h-[70vh]" : "aspect-video"}><iframe src={getYouTubeEmbedUrl(watchingVideo.url) || undefined} title={watchingVideo.title} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div><div className="flex items-center justify-between gap-3 p-4 text-white"><p className="truncate text-sm font-semibold">{watchingVideo.title}</p><button onClick={() => setWatchingVideo(null)} className="rounded-lg border border-white/20 px-3 py-2 text-xs">Close</button></div></div>}<div className="space-y-4">{videos.map(video => <article key={video.id} className="overflow-hidden rounded-2xl bg-white shadow-sm"><button onClick={() => setWatchingVideo({ title: video.title, url: video.url })} className="relative block w-full text-left"><img src={video.thumbnail} alt={video.title} className="h-52 w-full object-cover" /><span className="absolute inset-0 flex items-center justify-center"><span className="flex h-14 w-14 items-center justify-center rounded-full bg-[#A41E14] text-white shadow-xl">▶</span></span><span className="absolute bottom-3 right-3 rounded bg-black/75 px-2 py-1 text-[10px] text-white">{video.duration}</span></button><div className="p-4"><p className="text-[10px] font-semibold uppercase tracking-widest text-[#A41E14]">{video.url.includes("/shorts/") ? "YouTube Shorts" : video.category}</p><h2 className="mt-1 font-bold">{video.title}</h2><p className="mt-1 text-xs text-gray-500">{video.views} views</p><button onClick={() => setWatchingVideo({ title: video.title, url: video.url })} className="mt-3 text-sm font-semibold text-[#A41E14]">Play in app</button></div></article>)}</div></div>
        )}
